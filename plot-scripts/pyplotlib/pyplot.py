...
class Plotter():
    ...
    def set_xaxis(self,xlabel='X-axis',xrange=(0,1000)):
        self.xlabel=xlabel
        self.xrange=xrange
    ...
    def hist(self,histo,color='ROOT.kBlue',label='name',stack=False,fill=False,lwidth=2,ls=0.7,lstyle='ROOT.kSolid',legendStyle='lf',scale=1.0,density=False,isData=False):
        ...
        ##label
        if self.xrange==None:self.xrange=(h.GetBinLowEdge(1),h.GetNbinsX()*h.GetBinWidth(1))
    ...
    def Draw(self,logY=True,logX=False,truncate_xrange=False,rebin=1,unc_fstyle=1003,unc_fcolor=ROOT.kGray,extraTextOffset=0.08):
        ...
        stack_legend=sorted(stack_legend,key=lambda x:x[-1].Integral(),reverse=True)
        signal_legend=sorted(signal_legend,key=lambda x:x[-1].Integral(),reverse=True)
        if(self.pubStyle):
            if self.h_data!=None:
                self.mainleg.AddEntry(self.h_data,f"Data",'ep')
            ...
            if self.h_data!=None:
                self.mainleg.AddEntry(self.h_data,f"Data [{self.h_data.Integral():.3e}]",'ep')
            for item in stack_legend:
                label,histo=item[0],item[-1]                
                self.mainleg.AddEntry(histo,f"{label} [{histo.Integral():.3e}]",'f')
            for item in signal_legend:
                label,histo=item[0],item[-1]                
                self.mainleg.AddEntry(histo,f"{label} [{histo.Integral():.3e}]",'l')
        ...
        ##DrawExperimentLabel
        expLabel,extraLabel,lumiLabel=setExperimentLabelStyle()
        ...
        expLabel.DrawLatex(left_margin,0.925,self.experiment) #CMS
        extraLabel.DrawLatex(left_margin+extraTextOffset,0.925,self.extraText) #Preliminary
        lumiLabel.DrawLatex(1-right_margin,0.925,self.lumi+" fb^{#minus1} ("+self.energy+")") #138 ifb (13 TeV)
